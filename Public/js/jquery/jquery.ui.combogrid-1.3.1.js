	...
	_create: function() {
		var self = this,
			...
			if (self.options.resetButton){
				this.element.after('<span class="ui-state-default ui-corner-all" id="cg-resetButton" onClick="$(\'#'+this.element.attr('id')+'\').val(\'\')"><span class="ui-icon ui-icon-circle-close"></span></span>');
			}
		this.element
...
//		ul2.appendTo(ul);
	...
	_renderHeader: function(ul, colModel ) {
		...
		// Check if column is ordered or not to provide asc/desc icon
			if(col.columnName==self.cssCol){
				div.append('<div class="cg-colHeader" style="width:'+col.width+'%; text-align:'+col.align+'"><label class="cg-colHeader-label" id="'+ col.columnName +'">'
	...
	_renderMenu: function( ul, items, colModel ) {
		...
		$.each( items, function( index, item ) {
			if(self.options.alternate){
				self._renderItemAlt( ul, item, colModel );
			} else {
	...
	_renderItem: function( ul, item, colModel) {
		...
		if(col.width==undefined){col.width=100/colModel.length}
		if(col.align==undefined){col.align="center"}
			$("<div style='width:"+col.width+"%; text-align:"+col.align+"' class='cg-DivItem'>"+ item[col.columnName] +"</div>").appendTo(div);
		});
		div.append("</div>");
		return $( "<div class='cg-comboItem'></div>" ).data( "item.combogrid", item ).append(div).appendTo(ul);
	},
	_renderItemAlt: function( ul, item, colModel) {
		this.rowNumber++;
		div = $("<div class='cg-colItem'>");
		$.each( colModel, function( index, col ) {
			$("<div style='width:"+col.width+"%;' class='cg-DivItem'>"+ item[col.columnName] +"</div>").appendTo(div);
		});
		div.append("</div>");
